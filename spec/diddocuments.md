...
##### Ed25519
Ed25519 public keys must be converted to a verification method with a type of `CesrKey` and `publicKeyCesr` property whose value is the CESR representation of the public key. For example, a KERI AID with only the following inception event in its KEL:
  ...
  "verificationMethod": [
    {
      ...
      "publicKeyCesr": "DFkI8OSUd9fnmdDM7wz9o6GT_pJIvw1K_S21AKZg4VwK"
    }
  ]
```

##### Secp256k1
Secp256k1 public keys must be converted to a verification method with a type of `CesrKey` and `publicKeyCesr` property whose value is the CESR representation of the public key. For example, a KERI AID with only the following inception event in its KEL:
  ...
  "verificationMethod": [
    {
      ...
      "type": "CesrKey",
      "controller": "did:webs:example.com:EDP1vHcw_wc4M__Fj53-cJaBnZZASd-aMTaSyWEQ-PC2",
      "publicKeyCesr": "1AAAAmbFVu-Wf8NCd63B9V0zsy7EgB_ocX2_n_Nh1FCmgF0Y"
    }
  ]
```
