...
After the [[ref: host]] and optional path in a `did:webs` is a
...
when the [[ref: host]] portion of one of its DID is resolved. In fact, a `did:webs` can be resolved to a [[ref: DID document]]
