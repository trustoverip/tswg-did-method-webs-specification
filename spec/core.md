...
1. As with `did:web`, this method reads data from whatever web server is referenced
when the [[ref: host]] portion of one of its DID is resolved.
1. In fact, a `did:webs` can be resolved to a [[ref: DID document]]
...
1. As with `did:web`, the location of the [[ref: DID document]] MUST be determined by transforming the DID to an
...
1. The `did:web` version of the DIDs are the same (minus the `s`) and point
