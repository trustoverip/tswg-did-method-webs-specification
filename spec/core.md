...
* `did:webs:w3c-ccg.github.io:user:alice:12124313423525`
  ...
  * KERI event stream URL would look like: `https://w3c-ccg.github.io/user/alice/12124313423525/keri.cesr`
...
1. The following resolution paths that `did:webs` identfiers SHALL leverage to help in the face of resolution uncertainty includes:
    ...
    1. When a `did:webs` DID is permanently moved to some other location the resolver MAY redirect to any other `equivalentId` [[ref: designated aliases]].
        ...
        1. An `equivalentId` entry of the old location SHOULD remain for historical purposes and be anchored to the KERI event stream using [[ref: designated aliases]]. See section [Use of `equivalentId`](#use-of-equivalentid) for more details.
...
1. Resolving a `did:webs` DID MUST follow these steps:
    ...
    1. MUST process the KERI event stream using [KERI Rules] to verify it, then derive the `did:webs` [[ref: DID document]] by processing the KERI event stream according to section [DID Documents](#did-documents).
...
1. Updates to the KERI event stream that relate to the `did:webs` DID MUST be reflected in the DID Document as soon as possible.
